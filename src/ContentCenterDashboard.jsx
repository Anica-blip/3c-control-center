              ...
              React.createElement('div', {
                ...
                React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '15px' } },
                  ...
                  React.createElement('input', {
                    ...
                    onChange: (e) => setEditingCharacter(prev => ({ ...prev, image: e.target.value })),
                ...
                React.createElement('textarea', {
                  ...
                  placeholder: 'Character bio and expertise',
                  style: {
                    padding: '12px',
                    border: '1px solid #c4b5fd',
                    borderRadius: '6px',
...
function Settings() {
  ...
  const [editingTelegram, setEditingTelegram] = React.useState(null);
  ...
  const [editingCharacter, setEditingCharacter] = React.useState(null);
  ...
  const removePlatform = (id) => {
    ...
    if (editingPlatform && editingPlatform.id === id) {
      setEditingPlatform(null);
    }
  };
  ...
  const editTelegram = (telegram) => {
    setEditingTelegram({ ...telegram });
  };

  const saveTelegram = () => {
    if (!editingTelegram.name.trim() || !editingTelegram.channel_group.trim()) return;
  ...
  const removeTelegram = (id) => {
    ...
    if (editingTelegram && editingTelegram.id === id) {
      setEditingTelegram(null);
    }
  };
  ...
  const saveCharacter = () => {
    if (!editingCharacter.name.trim() || !editingCharacter.bio.trim()) return;
    const updatedCharacter = {
      ...editingCharacter,
      image: editingCharacter.image || `https://via.placeholder.com/80x80/7c3aed/ffffff?text=${editingCharacter.name.charAt(0)}`
    };
          ...
          ...telegramChannels.map(telegram =>
            editingTelegram && editingTelegram.id === telegram.id ?
              ...
              React.createElement('div', {
                ...
                React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '15px' } },
                  React.createElement('select', {
                    value: editingTelegram.type,
                    onChange: (e) => setEditingTelegram(prev => ({ ...prev, type: e.target.value })),
                    style: {
                      padding: '10px',
                      border: '1px solid #67e8f9',
                      borderRadius: '6px',
