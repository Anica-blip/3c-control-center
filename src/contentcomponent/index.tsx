...
export default function ContentComponent() {
  ...
  const handleSavePost = async (postData: Omit<ContentPost, 'id' | 'createdDate'>) => {
    try {
      ...
      // Check if editing existing post or creating new
      const postDataWithId = postData as any;
      
      if (postDataWithId.id) {
        // UPDATE existing post
        const updatedPost = await supabaseAPI.updateContentPost(postDataWithId.id, postData);
        setSavedPosts(prev => prev.map(post => 
          post.id === postDataWithId.id ? updatedPost : post
        ));
        setEditingPost(null);
