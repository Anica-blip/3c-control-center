...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!fontData || typeof fontData !== 'object') {
      return res.status(400).json({ error: 'Invalid font data provided' });
    }

    if (!fontData.name || !fontData.category || !fontData.usage) {
      return res.status(400).json({ error: 'Name, category, and usage are required' });
    }
    ...
    // Find the Typography System database
    const databaseId = await findDatabase(token, pageId, 'Typography System');
    ...
    const response = await fetch('https://api.notion.com/v1/pages', {
      ...
      body: JSON.stringify({
        ...
        properties: {
          ...
          'Weight Range': { rich_text: [{ text: { content: fontData.weight || '400-600' } }] },
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: `Failed to save font: ${response.status}` });
    }
