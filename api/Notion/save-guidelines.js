...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!section || typeof section !== 'string') {
      return res.status(400).json({ error: 'Section is required and must be a string' });
    }
    ...
    // Find the Brand Guidelines database
    const databaseId = await findDatabase(token, pageId, 'Brand Guidelines');
    ...
    if (typeof content === 'string') {
      ...
      if (content.dos && content.donts) {
        // Handle logo guidelines format with dos and donts
        contentText = `DO:\n${content.dos.join('\n')}\n\nDON'T:\n${content.donts.join('\n')}`;
      } else {
      ...
      body: JSON.stringify({
        ...
        properties: {
          'Section': { 
            title: [{ 
              text: { 
                content: `${section.charAt(0).toUpperCase() + section.slice(1)} Guidelines` 
              } 
            }] 
          },
          'Content': { 
            rich_text: [{ 
              text: { 
                content: contentText.length > 2000 ? contentText.substring(0, 2000) + '...' : contentText 
              } 
            }] 
          },
          'Type': { 
            select: { 
              name: `${section.charAt(0).toUpperCase() + section.slice(1)} Usage` 
            } 
          },
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: `Failed to save guidelines: ${response.status}` });
    }
